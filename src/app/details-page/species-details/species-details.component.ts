...
export class SpeciesDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.speciesDetailsSubscription = this.store
      .select(getSpeciesDetail)
      .subscribe((data) => {
        if (data) {
  ...
  fetchHomeWorldDetails() {
    this.httpHomeworldSubscription = this.http
      .get(this.speciesDetails.homeworld)
      .subscribe((data: any) => {
        if (data) {
  ...
  fetchSpeciesDetails() {
    this.httpSpeciesSubscription = this.http
      .get(BASE_URI + 'species/' + this.dataId)
      .subscribe((data: any) => {
        if (data) {
          this.store.dispatch(
            AppApiActions.displaySpeciesDetails({ species: data })
          );
        }
      });
  }
