...
export class CharacterDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.characterDetailSubscription = this.store
      .select(getCharacterDetail)
      .subscribe((data) => {
        if (data) {
  ...
  fetchCharacterDetails() {
    this.httpSubscription = this.http
      .get(BASE_URI + 'people/' + this.dataId)
      .subscribe((data: any) => {
        if (data) {
          this.store.dispatch(
            AppApiActions.displayCharacterDetails({ character: data })
          );
        }
      });
  }
  ...
  fetchHomeWorldDetails() {
    this.httpHomeworldSubscription = this.http
      .get(this.characterDetails.homeworld)
      .subscribe((data: any) => {
        if (data) {
